(function ($) {
	...
	$.fn.nextpage.defaults = {
		...
		offsetY : 100, //默认的Y轴偏移量（add lw 2015-5-19）
		...
		urlFormater : function (options, url, pageNext) {
			...
			var c = tmpUrl.charAt(tmpUrl.length - 1);
			if('/' == c) {
				tmpUrl = tmpUrl.substr(0, tmpUrl.length - 1); // 去除斜杠
				c = tmpUrl.charAt(tmpUrl.length - 1);
			}
			if(options.urlForceSplite || '/o' == tmpUrl.substr(tmpUrl.length - 2)) {
				tmpUrl += '-';
			} else if(null == c.match(/\d/)) {
				tmpUrl += '/';
			} else {
				if(null == tmpUrl.match(/^.*\/[\d\-]+$/)) {
					tmpUrl += '-';
				} else {
					tmpUrl += '/';
				}
			...
			tmpUrl += 'n' + pageNext;
			if(options.urlFormatNum) tmpUrl += '-aj' + options.urlFormatNum;
		...
		htmlLoad : '<img src="http://img02.fangyou.com/new-mobile/images/loding.gif" width="16" height="16">正在加载更多信息', // 加载提示
